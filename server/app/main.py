...
ITEMS: Dict[str, Dict[str, Any]] = {
    ...
    "water_bottle": {
        "id":"water_bottle","title":"Бутылка воды",
...
def _combine_use(left_id:str,right_id:str,target:Optional[str]=None)->List[Event]:
    """Зажигалка + дезодорант => конус огня с расходом обоих зарядов."""
    ev=[]
...
def post_intent(intent: Intent) -> List[Event]:
    ...
    if t=="TALK":
        ...
        # здесь пока мок-ответ; позже заменим на вызов /talk
    ...
    if t=="USE_ITEM":
        item_id = p.get("item_id")
        target = p.get("target_id")
        # предмет должен быть в одной из рук
...
# -------- /talk endpoint (на будущее LLM), сейчас мок ----------
...
def talk(input: TalkIn):
    # Позже: собрать контекст NPC, вызвать LLM.
